...
final class TemporalAlbumSynchronizer {
    private let config: PostgresConfig
    private let folderName: String
    private let namePattern: String
    private let dateFormatter: DateFormatter
    private let clusterStore: TemporalClusterStore
    ...
    init(config: PostgresConfig, restoreRemovals: Bool = false, dangerRemove: Bool = false) {
        self.config = config
        self.folderName = config.temporalAlbumFolderName ?? "Events"
        self.namePattern = config.temporalAlbumNamePattern ?? "{name} {start} – {end}"
        self.clusterStore = TemporalClusterStore(config: config)
        let formatter = DateFormatter()
        formatter.dateFormat = "yyyy-MM-dd"
        formatter.timeZone = TimeZone(secondsFromGMT: 0)
        self.dateFormatter = formatter
        self.photoLibrary = PhotoLibraryAdapter()
        self.overrideStore = AlbumSyncOverrideStore(scope: .temporal)
    ...
    func run() throws -> String {
        ...
        guard !clusters.isEmpty else { return "No temporal clusters to sync." }
        ...
        components.append("Touched \(touchedAlbumKeys.count) temporal albums in folder '\(folderName)'")
        if totalAdded > 0 {
